...
static void mirp_farr_interval(arb_t * f,
                               int lmn1, int lmn2,
                               const arb_t xyz1, const arb_t xyz2,
                               slong working_prec)
{
    int i, j, k;
...
static void mirp_G_interval(arb_t G, arb_t fp, arb_t fq,
                            ...
                            const arb_t gammap, const arb_t gammaq, const arb_t gammapq,
                            slong working_prec)
{
    arb_t tmp1, tmp2;
    arb_init(tmp1);
...
void mirp_eri_interval_core_(arb_t result,
                             const int * lmn1,
                             const int * lmn2,
                             const int * lmn3,
                             const int * lmn4,
                             const arb_t gammap, const arb_t gammaq, const arb_t gammapq,
                             const arb_t * PA, const arb_t * PB,
                             const arb_t * QC, const arb_t * QD,
                             const arb_t * PQ, const arb_t * F,
                             slong working_prec)
{
                              ...
                              slong working_prec)
{
     ...
     */
    mirp_eri_interval_core_(result, lmn1, lmn2, lmn3, lmn4,
    ...
    for(size_t i = 0; i < ncart1; i++)
    {
        ...
        for(size_t j = 0; j < ncart2; j++)
        {
            ...
            for(size_t k = 0; k < ncart3; k++)
            {
                ...
                {
                    mirp_eri_interval_core_(result[idx], lmn1, lmn2, lmn3, lmn4,
                                            gammap, gammaq, gammapq,
                                            PA, PB, QC, QD, PQ, F,
                                            working_prec);

                    /* apply the prefactor */
                    arb_mul(result[idx], result[idx], tmp1, working_prec);
                    idx++;
