...
rm -f mirp_testmatrix.txt

ALL_DEPS_ARCH="haswell"
ALL_COMP_MOD="gcc/7.2.0 gcc/6.4.0 gcc/5.4.0 gcc/4.9.4 clang/4.0.1 clang/5.0.0 intel/2017u4"
    ...
    for DEPS_ARCH in $ALL_DEPS_ARCH; do
      for OMP in $ALL_OMP; do
        for BTYPE in $ALL_BTYPES; do
          echo "${COMP_MOD} ${CMAKE_MOD} ${DEPS_ARCH} ${OMP} ${BTYPE}" >> mirp_testmatrix.txt
        done
      done
    done
  done
